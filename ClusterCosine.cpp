...
//-------------------------------------------------------------------MENU FUNCTION-----------------------------------------------------------------------------
void ClusterCosine::ClusterCosine_Menu(ifstream& infile,ifstream& cfile, ofstream& outfile)
{
	cout << "ClusterCosine_Menu" << endl;
	...
	if (infile.is_open())
	{
		cosine = new Cosine(hash_functions ,hash_tables, dimensions);
		while(getline(infile, line) && counter < MAXIMUM)     //Start reading items and put them at lists
		{
			Cosine_Reader(&line[0], counter);
			for(int i=0; i <= hash_tables - 1; i++)
				cosine->Cosine_Reader(&line[0], i, counter);		//for LSH/DBH
			counter++;
		}
	}
