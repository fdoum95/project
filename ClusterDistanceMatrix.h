#ifndef ClusterDistanceMatrix_H
#define ClusterDistanceMatrix_H

#include <fstream>
#include <iostream>
#include "DistanceMatrix.h"
#include "Cluster.h"
#include "List.h"
#include "Node.h"

class ClusterDistanceMatrix
{
	private:
		int k;			//number of clusters
		int nItems;
		
		int hash_functions;
		int hash_tables;
		int clarans_franction;
		int clarans_iterations;		
		
		int Number_of_Items;
		bool *Objective_Function_Updated;
		long int Objective_Function_Value;
		char optionInit;
		char optionAssign;
		char optionUpdate;
		char metric;
		
		double **Distance_Table;
		List<int> *all_items_list;
		List<int> *clara_list;
		Cluster<int> *cluster;
		
		int **table;
		
		double *Silhouette_table;
		
		DistanceMatrix *dmatrix;
	public:
		ClusterDistanceMatrix(int, int, int, int, int, int);
		~ClusterDistanceMatrix();
		
		int get_ClusterNumber();
		int get_ItemsNumber();
		
		void ClusterDistanceMatrix_Menu(std::ifstream& infile,std::ifstream& cfile, std::ofstream& outfile, int **);
			
		void Lloyd_Menu(std::ofstream& outfile);
		void Clarans_Menu(std::ofstream& outfile);
		void Clara_Menu(std::ofstream& outfile);
				
		void DistanceMatrix_Reader(int, int);
		
		void DistanceMatrix_Cluster_Initialization(char option = '1');
		
		void Distance_Table_Calculation();
		double Get_Distance(int , int );
		
		void KmedoidsInitialization();
		void ConcentrateInitialization();
		
		int Cluster_DistanceMatrix_Distance(Node<int>* cnode, Node<int>* inode);
		
		void Assignment_Initialization();
		
		void Assignment();
		
		void PAM_Assignment();
		void LSH_Assignment();
		
		bool LSH_Keep_Update(bool *);
		
		bool IsCentroid(Node<int> *);
		
		long int Objective_Function();
		
		int Objective_Function_New_Centroid(int ClusterNumber, int ItemClusterNumber, Node<int> *new_centroid);
		
		void Update_Lloyd();
		
		void Update_Lloyd_Change_All_medoids();
		
		void Update_Clarans();
		
		void Update_Clarans_One_Swap();
		
		Node<int> * Cluster_Medoid_Calculation(int);
		bool Keep_Update();
		
		Node<int> * get_second_best_Centroid(int, Node<int> *);
		
		void Clarans_Swap(int new_cluster_number, int new_centroid_cluster_number, Node<int> *new_centroid);
		
		void printClusters();
		
		void Clara_Function();
		
		void Clara_New_Sample(int NumItems);
		
		void PAM_Update();
		
		void Silhouette(std::ofstream& outfile);
};

#endif
