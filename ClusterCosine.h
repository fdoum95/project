#ifndef ClusterCosine_H
#define ClusterCosine_H
...
class ClusterCosine
{
	private:
		...
		Cosine *cosine;
	public:
		ClusterCosine(int, int, int, int, int, int);
		~ClusterCosine();
		
		int get_ClusterNumber();
		int get_DimensionsNumber();
		
		void ClusterCosine_Menu(std::ifstream& infile,std::ifstream& cfile, std::ofstream& outfile);
			
		void Lloyd_Menu(std::ofstream& outfile);
		void Clarans_Menu(std::ofstream& outfile);
		void Clara_Menu(std::ofstream& outfile);
		
		void Cosine_Reader(char *, int);
		
		void Cosine_Cluster_Initialization(char);
		...
		double Cluster_Cosine_Distance(Node<double>* cnode, Node<double>* inode);
